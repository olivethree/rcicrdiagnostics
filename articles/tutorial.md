...
Experiment 2 reported per-condition means across time, trial-count, and
stimulus-count criteria; all were below 1.96, ranging from 0.54
(Traditional-RC at 90 trials) to 1.35 (Brief-RC pooled at 10 min)
(pp. 11–12). Their own interpretation (p. 13): *“infoVal scores were
relatively low, regardless of the condition, signalling that increasing
the number of trials may be beneficial to improve both metrics.”* So
“low individual infoVal” is the normal empirical finding for Brief-RC in
the only published methodological evaluation of it.
...
`stimulus`, `response`, `trait`. Read it with `readr` and clean it up
with `dplyr`: rename `subject` to the column name the package expects,
make sure participant ids are stored as text, and lower-case the trait
...
`p$patchIdx` schema introduced in `rcicr` 1.0.x. To make the legacy file
work without re-running stimulus generation, load it, copy the old
fields into a new `p` list, and save the result under a new filename:

``` r
load("rcic_seed_1_time_fev_05_2015_03_17.Rdata")  # brings in s, base_faces, etc.
...
The new `stimuli_modernised.RData` is what we pass to the diagnostics
below.

### 9.3. Running the diagnostic battery

Pick the columns the package needs and call
[`run_diagnostics()`](https://olivethree.github.io/rcicrdiagnostics/reference/run_diagnostics.md):
...
If your design crosses multiple within-subject conditions, it is worth
running
[`check_response_bias()`](https://olivethree.github.io/rcicrdiagnostics/reference/check_response_bias.md)
separately per condition: a producer who looks balanced overall might
still be heavily biased in one trait. Filter the data down to one trait
and check it:

``` r
competent_data <- raw |>
  filter(trait == "competent") |>
  select(participant_id, stimulus, response)

check_response_bias(competent_data, method = "2ifc")
```

To check all ten traits, repeat the same recipe per trait. The most
direct way is a `for` loop:
...
[`diagnose_infoval()`](https://olivethree.github.io/rcicrdiagnostics/reference/diagnose_infoval.md)
runs the full six-step protocol from §6.9 on one trait condition at a
time. Start with a single trait so you know what the call looks like:
...
To run all ten traits, wrap the same call in a `for` loop and store each
result in a named list:
...
for (tr in sort(unique(raw$trait))) {
  ...
  trait_results[[tr]] <- diagnose_infoval(
    sub,
    method    = "2ifc",
    rdata     = "stimuli_modernised.RData",
    iter      = 1000,
    face_mask = "FMNES_mask_fullface_256.jpg",
    seed      = 1L
  )
}
```
...
|----------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **Computing infoVal**                                                                                    | Use `ci$ci` (the `scaling` argument at CI generation is irrelevant)                                | infoVal must be computed from the raw CI. [`rcicr::computeInfoVal2IFC()`](https://rdrr.io/pkg/rcicr/man/computeInfoVal2IFC.html) reads `ci$ci` internally regardless of which `scaling` was passed at CI generation, so the `scaling` choice does not affect the returned *z*-score. Do not pass `ci$scaled` to a hand-rolled infoVal computation. |
