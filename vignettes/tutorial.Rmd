...
**They return a `"skip"` result for Brief-RC** (see Section 6.9 for why).
...
**`"briefrc"`**: Following Schmitz et al. (2024), each trial is one
row. `stimulus` is the pool id of the chosen noise pattern, and
`response` is `+1` if the participant picked the oriented version
(`base + noise`) or `-1` if the inverted version (`base − noise`).
The unchosen alternatives are not recorded (see §3.4).
...
2.  **With replacement at the presentation level (the necessary path
    ...
    1500-item pool. (Schmitz et al. (2024) do not explicitly report
    using this regime; both their experiments fit the
    no-display-repeats reading at the noisy-face level.)
...
`infoval()` picks the right reference-distribution sampling
regime automatically: it samples without replacement when the
producer's trial count is at most the pool size, and with replacement
otherwise. This is the calibration fix the function was originally
written to address (described in §6.9.1). Users who designed their
task with a custom regime should still pass their actual trial count
via the `trial_counts` argument; the function does the rest.

**InfoVal reference distributions outside this package.** The
original `rcicr::computeInfoVal2IFC()` builds its reference at the pool size
regardless of the producer's actual trial count, which is correct for
2IFC (where they coincide) but biases Brief-RC infoVal downward. If
you are using stock `rcicr` rather than this package's `infoval()`,
...
2IFC needs the rcicr `.RData` and the base image; Brief-RC needs a
noise matrix and the base image.
...
Section 6.9.2 (`diagnose_infoval()`) accepts an optional logical mask
that restricts the infoVal computation to a subset of pixels (for
example, the face region). Three ways to obtain one:
...
nothing has failed; you just haven't handed `run_diagnostics()` the
extra inputs those checks need. Section 7.1 walks through exactly what
to pass for each one so you can progressively unlock them.
...
from Sections 6.1–6.8 run without any external files. The three infoVal-
...
`compute_infoval_summary()` is a thin wrapper around the original
`rcicr` package and inherits its 2IFC-only scope; calling it with
`method = "briefrc"` returns a `"skip"` result. For Brief-RC
infoVal, use `diagnose_infoval()` (§6.9.2), which is built on the
in-package `infoval()` and supports both paradigms with a reference
...
**fail** status based on per-participant z alone. As §6.9.1 explains,
...
`diagnose_infoval()` (§6.9.2) for the full verdict.

### 6.9.1. Why per-producer infoVal often looks "low"

A common moment of confusion in RC analysis is opening
`compute_infoval_summary()`'s output and seeing per-producer z below
the 1.96 threshold, sometimes negative, even though data collection
went smoothly. A few facts about infoVal are worth knowing before
...
What `diagnose_infoval()` automates:

- The per-trial-count reference (fixes the pool-size calibration bug
  for Brief-RC).
...
- Optional face masking via `face_mask()`, matching Schmitz's
  geometry.
...
target, paradigm, and trial count, and the published baselines above
are the honest yardsticks.

### 6.9.2. `diagnose_infoval()`: guided low/negative-z diagnostic

`diagnose_infoval()` is the recommended entry point when individual
infoVal looks low and you want to know whether the data is broken or
merely structurally diluted. It walks through six checks and produces
interpretation bullets, plus rich `$data` for plotting.
...
`diagnose_infoval()` uses an in-package implementation of Schmitz's
`genMask()` algebra and the per-trial-count reference distribution;
no `rcicr` install is required. Pass `noise_matrix = ...`. For
`method = "2ifc"`, the function calls `rcicr` to (a) compute
per-producer CIs from `stimuli_params` and `p` via
`rcicr::batchGenerateCI2IFC()`, and (b) reconstruct each pool item's
noise pattern via `rcicr::generateNoiseImage()` so the reference
distribution can be simulated; `rcicr` therefore needs to be
installed for this path. The reconstruction adds a few seconds for a
typical `n_pool = 770` pool.
...
through an in-package implementation; see Section 6.9.2.
