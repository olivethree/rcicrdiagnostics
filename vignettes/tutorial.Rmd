...
`rcicr::computeInfoVal2IFC()` builds its reference at the pool size
...
expose Brief-RC-specific CI or infoVal machinery, and a correct
Brief-RC infoVal needs a reference distribution matched to each
participant's trial count rather than the pool size stored in the
rdata. Calling `compute_infoval_summary(method = "briefrc")` returns
a `"skip"` result with this explanation; proper Brief-RC infoVal is
