...
`trial`, `stimulus`, `response`, `trait`. Rename to the package's
defaults and lower-case the trait labels:
...
`p$patchIdx` schema introduced in `rcicr` 1.0.x. The two-line
aliasing below makes a legacy rdata file work with the package
...
`check_response_bias()` separately per condition. A producer who is
balanced overall might still be heavily biased in one trait condition:

```{r oliveira-trait-bias, eval = FALSE}
trait_bias <- lapply(
...
`diagnose_infoval()` runs the full six-step protocol from §6.9 against
one trait condition at a time. For multi-condition studies, loop and
share the modernised rdata across calls:

```{r oliveira-infoval, eval = FALSE}
