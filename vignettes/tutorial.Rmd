...
the 2IFC path to the canonical `rcicr` package (Dotsch, 2016).
...
`check_stimulus_alignment` step. The infoVal-based checks
(Sections 6.9–6.11) will each return a `"skip"` result for Brief-RC
regardless of what you pass — canonical `rcicr` does not ship Brief-RC
infoVal machinery. See Section 6.9 for the full explanation.

**On the name**: `run_diagnostics()` runs *every check whose required
inputs are available*, not "every check unconditionally". The printed
`Skipped checks` list is how it keeps that honest — nothing runs
halfway, nothing errors silently. Treat the list as a to-do: each line
tells you exactly what to add to the call to unlock that check.
