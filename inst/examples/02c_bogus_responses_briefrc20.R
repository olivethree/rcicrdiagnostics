# 02c_bogus_responses_briefrc20.R -------------------------------------------
# Generate a synthetic Brief-RC 20-alternative response dataset.
#
# Schmitz, Rougier, & Yzerbyt (2024) describe a Brief-RC variant where 20
# noisy faces are shown per trial (10 oriented + 10 inverted) instead of
# 12. The row format and response semantics are identical to the 12-alt
# version; only the per-trial display count differs.
#
# Outputs (written to OUTPUT_DIR):
#   responses_briefrc20.csv       # participant_id, trial, stimulus, response, rt
#   participants_briefrc20.csv    # truth table incl. prefer_sign for biased
...
# ---- helpers (identical structure to 12-alt; only n_alternatives differs) -
...
resp_path <- file.path(OUTPUT_DIR, "responses_briefrc20.csv")
part_path <- file.path(OUTPUT_DIR, "participants_briefrc20.csv")
data.table::fwrite(responses,    resp_path)
data.table::fwrite(participants, part_path)

message(sprintf(
  "Brief-RC 20 responses: %d rows, %d participants, %d trials total -> %s\nTruth table -> %s",
