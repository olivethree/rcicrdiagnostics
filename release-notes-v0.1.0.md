...
- **User’s guide vignette** covering installation, data format
  requirements, every exported function with runnable examples, report
  interpretation, and a dedicated chapter on CI scaling decisions (which
  statistic to use for which downstream analysis: infoVal, pixel
  correlations, rating-task stimuli, figures).
...
This release will be archived on Zenodo and receive a DOI; the DOI will
be added to `inst/CITATION` and the README citation section in a
subsequent commit.
