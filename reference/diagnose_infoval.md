...
report 54-68% of 2IFC participants clearing 1.96 on perceived gender
(median 3-4); Schmitz et al. (2024) report Brief-RC infoVals
systematically below 1.96 across all conditions in both experiments. The
reportable summary is therefore context-specific: per-producer z is
useful for exclusion decisions on individual cases, while the group-mean
CI's z is often the more defensible aggregate when individual-level z is
noisy.
